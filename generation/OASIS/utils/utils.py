...
class results_saver():
    def __init__(self, opt):
        ...
        self.path_orig = os.path.join(path, "orig")
        self.path_to_save = {"label": self.path_label, "image": self.path_image, "orig": self.path_orig}
        os.makedirs(self.path_label, exist_ok=True)
        os.makedirs(self.path_image, exist_ok=True)
        os.makedirs(self.path_orig, exist_ok=True)
        self.num_cl = opt.label_nc + 2

    def __call__(self, label, generated, name, image=None, colorize_labels=True):
        assert len(label) == len(generated)
        for i in range(len(label)):
            im = tens_to_lab(label[i], self.num_cl, colorize_labels)
            self.save_im(im, "label", name[i])
            im = tens_to_im(generated[i]) * 255
            self.save_im(im, "image", name[i])
            if image is not None:
                im = tens_to_im(image[i]) * 255
                self.save_im(im, "orig", name[i])
...
class losses_saver():
    def __init__(self, opt):
        self.name_list = ["Generator", "Vgg", "Laplacian", "D_fake", "D_real", "LabelMix"]
        self.opt = opt
        ...
        for name in self.name_list:
            if opt.continue_train and not opt.with_new_data and os.path.exists(self.path+"/losses.npy"):
...
def tens_to_lab(tens, num_cl, colorize=True):
    if colorize:
        label_tensor = Colorize(tens, num_cl)
        label_numpy = np.transpose(label_tensor.numpy(), (1, 2, 0))
        return label_numpy
    else:
        return torch.argmax(tens, dim=0, keepdim=True)[0].cpu().numpy()
