...
def add_all_arguments(parser, train):
    ...
    parser.add_argument('--cbs', action='store_true',
                        help='train with gaussian filters(see Curriculum by Smoothing (NeurIPS 2020))')
    ...
    parser.add_argument('--upsample_mode', type=str, default='nearest',
                        help='upsample mode for generator (see https://pytorch.org/docs/stable/generated/torch.nn.Upsample.html)')
    ...
    if train:
        ...
        parser.add_argument('--no_load_opt', action='store_true',
                            help='if specified, not load options from previous training')
        ...
        parser.add_argument('--add_checker_loss', action='store_true', help='if specified, add Checkerboard loss')
        parser.add_argument('--lambda_checker', type=float, default=10.0, help='weight for Checkerboard loss')

        parser.add_argument('--add_lap_loss', action='store_true', help='if specified, add Laplacian Pyramid loss')
        parser.add_argument('--lambda_lap', type=float, default=10.0, help='weight for Laplacian loss')
        ...
        parser.add_argument('--test_all', action='store_true', help='if specified test on train and test parts')
        parser.add_argument('--save_orig', action='store_true',
                            help='if specified save original(golden) image during testing')
        parser.add_argument('--colorize_labels', action='store_true',
                            help='if specified save label images in 3-channel(RGB) format, otherwise use one channel')
    return parser
