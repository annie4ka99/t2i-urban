...
#--- create models ---#
if opt.cbs:
    model = models.OASIS_model_CBS(opt)
else:
...
std = 2.0
std_decay_factor = 0.9
std_decay_epoch = 5

for epoch in range(start_epoch, opt.num_epochs):
    if opt.cbs and epoch % std_decay_epoch == 0:
        model.module.update_gaussian_kernels(std=std)
        std *= std_decay_factor
