...
import models.generator_cbs as generators_cbs
import models.discriminator_cbs as discriminators_cbs
import os
import copy
import torch
...
from models.lap_pyramid_loss import LapLoss
...
class OASIS_model(nn.Module):
    def __init__(self, opt):
        ...
        if opt.phase == "train":
            ...
            # if opt.add_checker_loss:
            #     self.Checker_loss = losses.CheckerboardLoss()
            if opt.add_lap_loss:
                self.Laplacian_Loss = LapLoss()
    ...
    def forward(self, image, label, mode, losses_computer):
        ...
        if mode == "losses_G":
            ...
            if self.opt.add_vgg_loss:
                ...
                loss_G_vgg = None
            # if self.opt.add_checker_loss:
            #     loss_G_checker = self.opt.lambda_checker * self.Checker_loss(fake, for_real=True)
            #     loss_G += loss_G_checker
            # else:
            #     loss_G_checker = None
            if self.opt.add_lap_loss:
                loss_G_lap = self.opt.lambda_lap * self.Laplacian_Loss(fake, image)
                loss_G += loss_G_lap
            else:
                loss_G_lap = None
            return loss_G, [loss_G_adv, loss_G_vgg, loss_G_lap]
...
class OASIS_model_CBS(nn.Module):
    def __init__(self, opt):
        super(OASIS_model_CBS, self).__init__()
        self.opt = opt
        #--- generator and discriminator ---
        self.netG = generators_cbs.OASIS_Generator_CBS(opt)
        if opt.phase == "train":
            self.netD = discriminators_cbs.OASIS_Discriminator_CBS(opt)
        self.print_parameter_count()
        self.init_networks()
        #--- EMA of generator weights ---
        with torch.no_grad():
    ...
    def update_gaussian_kernels(self, kernel_size=3, std=1.0):
        self.netG.update_gaussian_kernels(kernel_size=kernel_size, std=std)
        self.netD.update_gaussian_kernels(kernel_size=kernel_size, std=std)
        if not self.opt.no_EMA:
            self.netEMA.update_gaussian_kernels(kernel_size=kernel_size, std=std)
