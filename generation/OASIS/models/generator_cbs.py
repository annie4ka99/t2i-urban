...
from utils.filters import get_gaussian_filter


class OASIS_Generator_CBS(nn.Module):
    def __init__(self, opt):
        ...
        self.upsample_kernels = nn.ModuleList([get_gaussian_filter(
            kernel_size=3,
            sigma=1.0,
            channels=self.channels[i+1],
        ) for i in range(self.opt.num_res_blocks-1)])


        self.init_W, self.init_H = self.compute_latent_vector_size(opt)
        self.conv_img = nn.Conv2d(self.channels[-1], 3, 3, padding=1)
        self.up = nn.Upsample(scale_factor=2, mode=opt.upsample_mode)
        self.body = nn.ModuleList([])
        for i in range(len(self.channels)-1):
            self.body.append(ResnetBlock_with_SPADE_CBS(self.channels[i], self.channels[i+1], opt))
    ...
    def update_gaussian_kernels(self, kernel_size=3, std=1.0):
        ...
        for i in range(self.opt.num_res_blocks - 1):
            self.upsample_kernels[i] = get_gaussian_filter(
                kernel_size=kernel_size,
                sigma=std,
                channels=self.channels[i+1],
            )
        ...
        for i in range(self.opt.num_res_blocks):
            self.body[i].update_gaussian_kernels(kernel_size=kernel_size, std=std)
    ...
    def forward(self, input, z=None):
        ...
        x = self.kernel(self.fc(x))
        for i in range(self.opt.num_res_blocks):
            x = self.body[i](x, seg)
            if i < self.opt.num_res_blocks-1:
                x = self.upsample_kernels[i](self.up(x))
        x = self.conv_img(F.leaky_relu(x, 2e-1))
        x = F.tanh(x)
        return x


class ResnetBlock_with_SPADE_CBS(nn.Module):
    ...
    def forward(self, x, seg):
        if self.learned_shortcut:
            ...
            x_s = x
        dx = self.kernel1(self.conv_0(self.activ(self.norm_0(x, seg))))
        dx = self.kernel2(self.conv_1(self.activ(self.norm_1(dx, seg))))
        out = x_s + dx
        return out
