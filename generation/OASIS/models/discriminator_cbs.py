...
class OASIS_Discriminator_CBS(nn.Module):
    ...
    def update_gaussian_kernels(self, kernel_size=3, std=1.0):
        for i in range(len(self.body_down)):
            self.body_down[i].update_gaussian_kernels(kernel_size=kernel_size, std=std)
            self.body_up[i].update_gaussian_kernels(kernel_size=kernel_size, std=std)
...
class residual_block_D(nn.Module):
    def __init__(self, fin, fout, opt, up_or_down, first=False):
        ...
        self.sampling_kernel = get_gaussian_filter(
            kernel_size=3,
            sigma=1.0,
            channels=self.fout,
        )
    ...
    def forward(self, x):
        x_s = self.shortcut(x)
        dx = self.kernel1(self.conv1(x))
        dx = self.kernel2(self.conv2(dx))
        if self.up_or_down < 0:
            dx = self.sampling(dx)
        out = x_s + dx
        return out
    ...
    def shortcut(self, x):
        if self.first:
            ...
            if self.up_or_down > 0:
                x = self.sampling_kernel(self.sampling(x))
            if self.learned_shortcut:
                x = self.conv_s(x)
            if self.up_or_down < 0:
                x = self.sampling(x)
