...
class CheckerboardLoss(nn.Module):
    def __init__(self, requires_grad=False):
        super(CheckerboardLoss, self).__init__()

        model = models.resnet50()
        num_ftrs = model.fc.in_features
        model.fc = nn.Linear(num_ftrs, 2)
        model.cuda()
        checkpoint = torch.load('checker_checkpoints/checkpoint_10.pth')
    ...
    def forward(self, x, for_real):
        if for_real:
            label = torch.ones(x.size()[0], dtype=torch.long, requires_grad=False, device='cuda:0')
        else:
            label = torch.zeros(x.size()[0], dtype=torch.long, requires_grad=False, device='cuda:0')
        fft_x = get_fft(x)
        out = self.model(fft_x)
...
def get_fft(tens):
    ...
    fft_img_flatten = torch.flatten(fft_img, start_dim=-2)
    fft_min = torch.quantile(fft_img_flatten, 0.05, dim=-1)
    fft_max = torch.quantile(fft_img_flatten, 0.95, dim=-1)

    fft_img = (fft_img - fft_min[:, :, None, None]) / (fft_max - fft_min)[:, :, None, None]
